...
public class MultiPolygon2D implements Domain2D, Polygon2D {
    ...
    public Polygon2D complement() {
        ArrayList<ClosedPolyline2D> reverseLines = new ArrayList<ClosedPolyline2D>(
                polylines.size());
    ...
    public MultiPolygon2D clip(Box2D box) {
        BoundarySet2D<?> boundary = Boundary2DUtils.clipBoundary(this
                .getBoundary(), box);
        ArrayList<ClosedPolyline2D> boundaries = new ArrayList<ClosedPolyline2D>(
    ...
    public MultiPolygon2D transform(AffineTransform2D trans) {
        ArrayList<ClosedPolyline2D> transformed = new ArrayList<ClosedPolyline2D>(
                polylines.size());
        for (ClosedPolyline2D line : polylines)
