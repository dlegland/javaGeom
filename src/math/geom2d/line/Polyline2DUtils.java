...
public abstract class Polyline2DUtils {
	 ...
	 * The resulting curve is continuous, but can self-intersect. It is
	 * composed of line segments, and circle arcs.
	 * @param polyline the source curve
	 * @param d the signed distance between the original curve and its
	 * parallel
	...
	createParallel(Polyline2D polyline, double d){
		...
		if(polyline instanceof ClosedPolyline2D){
			...
			if(Angle2D.getAngle(line0, line)>Math.PI ^ d<0){
				...
				result.addCurve(new CircleArc2D(v1, Math.abs(d), 
						circle.getPosition(p0), circle.getPosition(p1), d>0));
			}
		...
		while(iterator.hasNext()){
			...
			if(Angle2D.getAngle(line0, line)>Math.PI ^ d<0){
				...
				circle = new Circle2D(v1, Math.abs(d));
				result.addCurve(new CircleArc2D(v1, Math.abs(d), 
						circle.getPosition(p2), circle.getPosition(p1), d>0));
			}
	...
	createClosedParallel(ClosedPolyline2D polyline, double d){
