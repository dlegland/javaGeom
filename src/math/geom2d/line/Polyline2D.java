...
public class Polyline2D implements ContinuousOrientedCurve2D{
	...
	public double getWindingAngle(java.awt.geom.Point2D point) {
		...
		int n = points.size();
		for(int i=0; i<n-1; i++)
			angle += new LineSegment2D(points.get(i), points.get(i+1)).getWindingAngle(point);
	...
	public boolean isInside(java.awt.geom.Point2D pt) {
		...
		Point2D q0 = (Point2D)this.points.get(1);
		if(new StraightLine2D(q0, p0).isInside(pt))
			return false;

		Point2D p1 = this.getLastPoint();
		Point2D q1 = (Point2D)this.points.get(points.size()-2);
	...
	public Polyline2D getSubCurve(double t0, double t1){
		...
		for(int n=ind0+1; n<=ind1; n++)
			res.addPoint((Point2D) points.get(n));
	 ...
	 * contains only instances of Polyline2D. If the ellipse arc is not
	 * clipped, the result is an instance of
	 * CurveSet2D<Polyline2D> which contains 0 curves.
	 */
	...
	public boolean equals(Object object){
		if(!(object instanceof Polyline2D)) return false;
