...
/**
 * A PolyCurve2D is a set of piecewise smooth curve arcs, such that the
...
public class PolyCurve2D<T extends ContinuousCurve2D> extends CurveSet2D<T>
	...
	public PolyCurve2D<? extends ContinuousCurve2D> getReverseCurve(){
		ContinuousCurve2D[] curves2 = 
			new ContinuousCurve2D[curves.size()];
		int n=curves.size();
		for(int i=0; i<n; i++)
	 ...
	 * CurveSet2D<ContinuousCurve2D>, which 
	 * contains only instances of ContinuousCurve2D.
	 * If the PolyCurve2D is not clipped, the result is an instance of
	 * CurveSet2D<ContinuousCurve2D> which contains 0 curves.
	 */
	@Override
	public CurveSet2D<? extends ContinuousCurve2D> clip(Box2D box) {
		...
		CurveSet2D<ContinuousCurve2D> result =
			new CurveSet2D<ContinuousCurve2D> ();
