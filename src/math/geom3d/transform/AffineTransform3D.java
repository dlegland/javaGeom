...
public class AffineTransform3D implements Bijection3D {
	...
	public AffineTransform3D getInverseTransform(){
		...
		return new AffineTransform3D(
				...
				(m01*m12-m11*m02)/det, (m01*(m22*m13-m12*m23)+m02*(m11*m23-m21*m13)-m03*(m11*m22-m21*m12))/det, 
				(m20*m12-m10*m22)/det, (m00*m22-m20*m02)/det,
				(m10*m02-m00*m12)/det, (m00*(m12*m23-m22*m13)-m02*(m10*m23-m20*m13)+m03*(m10*m22-m20*m12))/det, 
				(m10*m21-m20*m11)/det, (m20*m01-m00*m21)/det,
				(m00*m11-m10*m01)/det, (m00*(m21*m13-m11*m23)+m01*(m10*m23-m20*m13)-m03*(m10*m21-m20*m11))/det);
	}
	...
	public Point3D transformPoint(Point3D src, Point3D dst){
		double coef[] = getCoefficients();
		if (dst==null)dst = new Point3D();	
		dst.setLocation(new Point3D(
			src.getX()*coef[0] + src.getY()*coef[1] + src.getZ()*coef[2] + coef[3],
			src.getX()*coef[4] + src.getY()*coef[5] + src.getZ()*coef[6] + coef[7],
			src.getX()*coef[8] + src.getY()*coef[9] + src.getZ()*coef[10] + coef[12]));
		return dst;
	}
	...
	public boolean equals(Object obj){
		...
		if(Math.abs(tab[0]-m00)>Shape3D.ACCURACY) return false;
		if(Math.abs(tab[1]-m01)>Shape3D.ACCURACY) return false;
		if(Math.abs(tab[2]-m02)>Shape3D.ACCURACY) return false;
		if(Math.abs(tab[3]-m03)>Shape3D.ACCURACY) return false;
		if(Math.abs(tab[4]-m10)>Shape3D.ACCURACY) return false;
		if(Math.abs(tab[5]-m11)>Shape3D.ACCURACY) return false;
		if(Math.abs(tab[6]-m12)>Shape3D.ACCURACY) return false;
		if(Math.abs(tab[7]-m13)>Shape3D.ACCURACY) return false;
		if(Math.abs(tab[8]-m20)>Shape3D.ACCURACY) return false;
		if(Math.abs(tab[9]-m21)>Shape3D.ACCURACY) return false;
		if(Math.abs(tab[10]-m22)>Shape3D.ACCURACY) return false;
		if(Math.abs(tab[11]-m23)>Shape3D.ACCURACY) return false;
		return true;
	}
