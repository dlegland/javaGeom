...
public class CheckPolyCirculinearCurve2DGetBuffer extends JPanel {
    ...
    public CheckPolyCirculinearCurve2DGetBuffer() {
        ...
        LineSegment2D line1 = new LineSegment2D(new Point2D(x0 + R, y0), new Point2D(x0 + 2 * R, y0));
        CircleArc2D arc1 = new CircleArc2D(center, 2 * R, 0, Math.PI / 2);
        LineSegment2D line2 = new LineSegment2D(new Point2D(x0, y0 + 2 * R), new Point2D(x0, y0 + R));
    ...
    public void paintComponent(Graphics g) {
        ...
        IDomain2D buffer = bc.computeBuffer(curve, 10);
