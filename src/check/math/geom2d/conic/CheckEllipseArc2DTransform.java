...
public class CheckEllipseArc2DTransform extends JPanel {
    ...
    public void paintComponent(Graphics g) {
        ...
        ICurveSet2D<? extends ICurve2D> clipped = ellipse.clip(box);
        g2.setColor(Color.CYAN);
        ellipse.draw(g2);
        g2.setColor(Color.BLUE);
        clipped.draw(g2);

        AffineTransform2D trans = AffineTransform2D.createLineReflection(new StraightLine2D(150, 150, 1, 0));
        g2.setColor(Color.CYAN);
        ellipse.transform(trans).draw(g2);
        g2.setColor(Color.BLUE);
        ICurveSet2D<? extends ICurve2D> transformed = clipped.transform(trans);
