...
public class DrawVennDiagram extends JPanel {
    ...
    public void paintComponent(Graphics g) {
        ...
        // circle arcs
        CircleArc2D cad0 = new CircleArc2D(cd0, cd0.position(pc3), cd0.position(pc0), true);
        CircleArc2D cad1 = new CircleArc2D(cd1, cd1.position(pc0), cd1.position(pc1), false);
        CircleArc2D cad2 = new CircleArc2D(cd2, cd2.position(pc1), cd2.position(pc2), true);
    ...
    private PolyCurve2D<CircleArc2D> createVennCurve(int level) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            tangents[i] = new StraightLine2D(points[i], c0.tangent(positions[i]));
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            circles[i] = new Circle2D(centers[i], centers[i].distance(points[i]));
            arcs[i] = new CircleArc2D(circles[i], circles[i].position(points[j]), circles[i].position(points[i]), i % 2 == 0);
        }
