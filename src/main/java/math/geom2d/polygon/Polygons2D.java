...
public final class Polygons2D {
     ...
     * @since 0.10.3
     */
    public final static SimplePolygon2D createRectangle(double x1, double y1, double x2, double y2) {
     ...
     * @since 0.9.1
     */
    public final static SimplePolygon2D createCenteredRectangle(Point2D center, double length, double width) {
        ...
        // create result polygon
        return new SimplePolygon2D(new Point2D[] { new Point2D(x1, y1), new Point2D(x2, y1), new Point2D(x2, y2), new Point2D(x1, y2), });
    }
     ...
     * @since 0.9.1
     */
    public final static SimplePolygon2D createOrientedRectangle(Point2D center, double length, double width, double theta) {
        // extract rectangle parameters
        double xc = center.x();
        double yc = center.y();
        double len = length / 2;
        ...
        return new SimplePolygon2D(new Point2D[] { new Point2D(-len * cot + wid * sit + xc, -len * sit - wid * cot + yc), new Point2D(len * cot + wid * sit + xc, len * sit - wid * cot + yc), new Point2D(len * cot - wid * sit + xc, len * sit + wid * cot + yc), new Point2D(-len * cot - wid * sit + xc, -len * sit + wid * cot + yc), });
    }
     ...
     * @since 0.9.1
     */
    public final static Point2D computeCentroid(IPolygon2D polygon) {
    ...
    /**
     * Computes the signed area of the polygon. Algorithm is taken from page: <a href="http://local.wasp.uwa.edu.au/~pbourke/geometry/polyarea/"> http://local.wasp.uwa.edu.au/~pbourke/geometry/polyarea/</a>. Signed area is positive if polygon is oriented counter-clockwise, and negative otherwise. Result is wrong if polygon is self-intersecting.
     * 
     * @return the signed area of the polygon.
     * @since 0.9.1
     */
    public final static double computeArea(IPolygon2D polygon) {
        double area = 0;
        for (LinearRing2D ring : polygon.contours()) {
            area += computeArea(ring);
        }
    ...
    /**
     * Computes the signed area of the linear ring. Algorithm is taken from page: <a href="http://local.wasp.uwa.edu.au/~pbourke/geometry/polyarea/"> http://local.wasp.uwa.edu.au/~pbourke/geometry/polyarea/</a>. Signed area is positive if linear ring is oriented counter-clockwise, and negative otherwise. Result is wrong if linear ring is self-intersecting.
     * 
     * @return the signed area of the polygon.
     * @since 0.9.1
     */
     ...
     */
    public final static int windingNumber(Collection<Point2D> vertices, Point2D point) {
    ...
    /**
     * Tests if a point is Left|On|Right of an infinite line. Input: three points P0, P1, and P2 Return: >0 for P2 left of the line through P0 and P1 =0 for P2 on the line <0 for P2 right of the line See: the January 2001 Algorithm "Area of 2D and 3D Triangles and Polygons"
     */
     ...
     */
    public final static IPolygon2D convexHull(Collection<? extends Point2D> points) {
     ...
     * 
     * @see IPolygon2D#buffer(double)
     */
    public final static ICirculinearDomain2D createBuffer(IPolygon2D polygon, double dist) {
     ...
     * 
     * @see IPolygon2D#clip(Box2D)
     */
    public final static IPolygon2D clipPolygon(IPolygon2D polygon, Box2D box) {
    ...
    private final static LinearRing2D convertContourToLinearRing(IContour2D contour) {
     ...
     */
    public final static IPolygon2D union(IPolygon2D polygon1, IPolygon2D polygon2) {
     ...
     */
    public final static IPolygon2D intersection(IPolygon2D polygon1, IPolygon2D polygon2) {
        // convert to GPCJ data structures
        Poly poly1 = convertToGpcjPolygon(polygon1);
        Poly poly2 = convertToGpcjPolygon(polygon2);
     ...
     */
    public final static IPolygon2D exclusiveOr(IPolygon2D polygon1, IPolygon2D polygon2) {
        // convert to GPCJ data structures
        Poly poly1 = convertToGpcjPolygon(polygon1);
        Poly poly2 = convertToGpcjPolygon(polygon2);
     ...
     * @since 0.9.1
     */
    public final static IPolygon2D difference(IPolygon2D polygon1, IPolygon2D polygon2) {
        // convert to GPCJ data structures
        Poly poly1 = convertToGpcjPolygon(polygon1);
        Poly poly2 = convertToGpcjPolygon(polygon2);
    ...
    private final static Poly convertToGpcjPolygon(IPolygon2D polygon) {
    ...
    private final static IPolygon2D convertFromGpcjPolygon(Poly poly) {
    ...
    private final static LinearRing2D convertFromGpcjSimplePolygon(Poly poly) {
