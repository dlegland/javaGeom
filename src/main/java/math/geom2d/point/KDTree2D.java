...
/**
 * A data structure for storing a great number of points. During construction of the tree, median point in current coordinate is chosen for each step, ensuring the final tree is balanced. The cost for retrieving a point is O(log n).<br>
 * The cost for building the tree is O(n log^2 n), that can take some time for large points sets.<br>
...
public class KDTree2D {
    ...
    private Node makeTree(List<Point2D> points, int depth) {
        ...
        return new Node(points.get(med), makeTree(points.subList(0, med), depth + 1), makeTree(points.subList(med + 1, n), depth + 1));
    }
    ...
    /**
     * Return either the same node as candidate, or another node whose point is closer.
     */
