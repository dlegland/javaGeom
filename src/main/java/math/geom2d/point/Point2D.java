...
public final class Point2D implements IPointShape2D, Serializable {
    ...
    /**
     * Creates a new point from polar coordinates <code>rho</code> and <code>theta</code>, from the given point.
     */
    ...
    /**
     * Creates a new point from polar coordinates <code>rho</code> and <code>theta</code>, from the position (x0,y0).
     */
    ...
    public static boolean isColinear(Point2D p1, Point2D p2, Point2D p3) {
        ...
        return Math.abs(dx1 * dy2 - dy1 * dx2) < IShape2D.ACCURACY;
    }
    ...
    public Point2D rotate(Point2D center, double theta) {
        ...
        double cot = cos(theta);
        double sit = sin(theta);
        return new Point2D(x * cot - y * sit + (1 - cot) * cx + sit * cy, x * sit + y * cot + (1 - cot) * cy - sit * cx);
    }
    ...
    public Point2D transform(CircleInversion2D inv) {
        ...
        double theta = Angle2DUtil.horizontalAngle(center, this);
