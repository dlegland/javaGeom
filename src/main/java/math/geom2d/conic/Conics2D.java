...
public class Conics2D {
    ...
    public final static IConic2D reduceConic(double[] coefs) {
        if (coefs.length < 6) {
        ...
        if (abs(b) < eps) {
            ...
            if (abs(a - c) < eps)
                ...
                theta0 = Angle2DUtil.formatAngle(atan2(b, a - c) / 2);
        ...
        if (abs(a1) < eps) {
            ...
            if (abs(d1) < eps) {
                ...
                if (delta >= 0) {
                    ...
                    Point2D center = new Point2D(0, ys).transform(AffineTransform2D.createRotation(theta0));
            ...
            if (abs(e1) < eps) {
                ...
                if (delta >= 0) {
                    ...
                    Point2D center = new Point2D(0, xs).transform(AffineTransform2D.createRotation(theta0));
                    return new ConicTwoLines2D(center, dist, theta0);
                } else
                    return null; // TODO: throw exception ?
            }
        ...
        if (at > 0 && bt > 0) {
            ...
            if (at > bt)
                ...
                return new Ellipse2D(center, sqrt(bt), sqrt(at), Angle2DUtil.formatAngle(theta0 + PI / 2));
        }
     ...
     * 
     * @param coefs
     *            an array of double with 6 coefficients
     * @param trans
     *            an affine transform
     * @return the coefficients of the transformed conic
     */
    public final static double[] transform(double[] coefs, AffineTransform2D trans) {
