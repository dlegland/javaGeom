...
public abstract class Curves2D {
     ...
     */
    public static ICurveSet2D<? extends ICurve2D> clipCurve(ICurve2D curve, Box2D box) {
     ...
     * <li>Remove intersections which do not cross (they only touch) the box boundary</li>
     * <li>Add portions of curves located between two intersections and inside of the box</li>
     * </ul>
     * </p>
     * <p>
    ...
    public static ICurveSet2D<IContinuousCurve2D> clipContinuousCurve(IContinuousCurve2D curve, Box2D box) {
        ...
        if (isLeftInfinite(curve)) {
            ...
            if (box.boundary().contains(point)) {
                ...
                while (Math.abs(pos - t0) < IShape2D.ACCURACY && iter.hasNext())
    ...
    public static ICurveSet2D<ISmoothCurve2D> clipSmoothCurve(ISmoothCurve2D curve, StraightLine2D line) {
        ...
        for (Point2D point : list) {
            ...
            if (Vector2D.isColinear(tangent, vector)) {
                ...
                if (Math.abs(curv) > IShape2D.ACCURACY)
                    continue;
            }
    ...
    public static boolean isLeftInfinite(ICurve2D curve) {
        // basic check
        if (curve.isBounded())
            return false;

        // extract the first smooth curve
        IContinuousCurve2D cont = curve.continuousCurves().iterator().next();
        ISmoothCurve2D smooth = cont.smoothPieces().iterator().next();

        // check first position of first curve
        return Double.isInfinite(smooth.t0());
    }

    public static boolean isRightInfinite(ICurve2D curve) {
        // basic check
        if (curve.isBounded())
            return false;
    ...
    public static IContinuousCurve2D getFirstContinuousCurve(ICurve2D curve) {
        if (curve instanceof IContinuousCurve2D)
            return (IContinuousCurve2D) curve;

        Collection<? extends IContinuousCurve2D> curves = curve.continuousCurves();
        if (curves.size() == 0)
            return null;
    ...
    public static IContinuousCurve2D getLastContinuousCurve(ICurve2D curve) {
        if (curve instanceof IContinuousCurve2D)
            return (IContinuousCurve2D) curve;
        IContinuousCurve2D res = null;
        for (IContinuousCurve2D continuous : curve.continuousCurves())
            res = continuous;
        return res;
    }

    public static ISmoothCurve2D getFirstSmoothCurve(ICurve2D curve) {
        if (curve instanceof ISmoothCurve2D)
            return (ISmoothCurve2D) curve;

        // Extract last continuous piece of the last continuous curve
        IContinuousCurve2D continuous = getFirstContinuousCurve(curve);
        if (continuous == null)
            return null;

        Collection<? extends ISmoothCurve2D> curves = continuous.smoothPieces();
        if (curves.size() == 0)
            return null;
    ...
    public static ISmoothCurve2D getLastSmoothCurve(ICurve2D curve) {
        if (curve instanceof ISmoothCurve2D)
            return (ISmoothCurve2D) curve;

        // Extract last continuous piece of the last continuous curve
        IContinuousCurve2D continuous = getLastContinuousCurve(curve);
        ISmoothCurve2D res = null;
        for (ISmoothCurve2D smooth : continuous.smoothPieces())
            res = smooth;
        return res;
    }
     ...
     */
    public static JunctionType getJunctionType(ICurve2D prev, ICurve2D next) {

        // Extract corresponding smooth curves
        ISmoothCurve2D smoothPrev = getLastSmoothCurve(prev);
        ISmoothCurve2D smoothNext = getFirstSmoothCurve(next);
        ...
        if (sp == 1 && sn == -1) {
            return abs(kappaPrev) < abs(kappaNext) ? JunctionType.SALIENT : JunctionType.REENTRANT;
        } else if (sp == -1 && sn == 1) {
            return abs(kappaPrev) > abs(kappaNext) ? JunctionType.SALIENT : JunctionType.REENTRANT;
        }
