...
public class PolyCurve2D<T extends IContinuousCurve2D> extends CurveArray2D<T> implements IContinuousCurve2D {
    ...
    private static Collection<ISmoothCurve2D> getSmoothCurves(ICurve2D curve) {
        ...
        throw new IllegalArgumentException("could not find smooth parts of curve with class " + curve.getClass().getName());
    }
     ...
     */
    @Override
    public PolyCurve2D<? extends IContinuousCurve2D> reverse() {
        // create array for storing reversed curves
        int n = curves.size();
        IContinuousCurve2D[] curves2 = new IContinuousCurve2D[n];
    ...
    public PolyCurve2D<? extends IContinuousCurve2D> subCurve(double t0, double t1) {
        ...
        for (ICurve2D curve : set.curves())
            subCurve.add((IContinuousCurve2D) curve);
    ...
    /**
     * Clip the PolyCurve2D by a box. The result is an instance of CurveSet2D<ContinuousCurve2D>, which contains only instances of ContinuousCurve2D. If the PolyCurve2D is not clipped, the result is an instance of CurveSet2D<ContinuousCurve2D> which contains 0 curves.
     */
    @Override
    public ICurveSet2D<? extends IContinuousCurve2D> clip(Box2D box) {
        ...
        for (ICurve2D curve : set.curves()) {
            if (curve instanceof IContinuousCurve2D)
                result.add((IContinuousCurve2D) curve);
        }
        return result;
    }
    ...
    public java.awt.geom.GeneralPath getGeneralPath() {
        ...
        for (IContinuousCurve2D curve : curves) {
            ...
            if (start.distance(current) > IShape2D.ACCURACY)
