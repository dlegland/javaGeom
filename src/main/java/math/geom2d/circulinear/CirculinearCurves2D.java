...
public class CirculinearCurves2D {
    ...
    public static ICirculinearCurve2D convert(ICurve2D curve) {
        ...
        if (curve instanceof IContinuousCurve2D) {
            ...
            IContinuousCurve2D continuous = (IContinuousCurve2D) curve;
            Collection<? extends ISmoothCurve2D> smoothPieces = continuous.smoothPieces();

            // prepare array of elements
            ArrayList<ICirculinearElement2D> elements = new ArrayList<ICirculinearElement2D>(smoothPieces.size());

            // class cast for each element, or throw an exception
            for (ISmoothCurve2D smooth : smoothPieces) {
                if (smooth instanceof ICirculinearElement2D)
                    elements.add((ICirculinearElement2D) smooth);
                else
                    throw new NonCirculinearClassException(smooth);
            }

            // create the resulting CirculinearContinuousCurve2D
            return new PolyCirculinearCurve2D<ICirculinearElement2D>(elements);
        }

        // If the curve is continuous, creates a CirculinearContinuousCurve2D
        if (curve instanceof ICurveSet2D<?>) {
            // extract smooth pieces
            ICurveSet2D<?> set = (ICurveSet2D<?>) curve;
            Collection<? extends IContinuousCurve2D> continuousCurves = set.continuousCurves();

            // prepare array of elements
            ArrayList<ICirculinearContinuousCurve2D> curves = new ArrayList<ICirculinearContinuousCurve2D>(continuousCurves.size());

            // class cast for each element, or throw an exception
            for (IContinuousCurve2D continuous : continuousCurves) {
                if (continuous instanceof ICirculinearContinuousCurve2D)
                    curves.add((ICirculinearContinuousCurve2D) continuous);
                else
                    curves.add((ICirculinearContinuousCurve2D) convert(continuous));
            }
     ...
     */
    public static double getLength(ICurveSet2D<? extends ICirculinearCurve2D> curve, double pos) {
        // init
        double length = 0;
     ...
     */
    public static double getPosition(ICurveSet2D<? extends ICirculinearCurve2D> curveSet, double length) {
        ...
        for (ICirculinearCurve2D curve : curveSet.curves()) {
     ...
     */
    public static Collection<Point2D> findSelfIntersections(ICirculinearCurve2D curve) {
        ...
        int n = elements.size();
        for (int i = 0; i < n - 1; i++) {
            ICirculinearElement2D elem1 = elements.get(i);
            for (int j = i; j < n; j++) {
                ICirculinearElement2D elem2 = elements.get(j);
    ...
    public static double[][] locateSelfIntersections(ICurveSet2D<? extends ICirculinearElement2D> curve) {
        ...
        int n = curve.size();
        for (int i = 0; i < n - 1; i++) {
            ICirculinearElement2D elem1 = curve.get(i);
            for (int j = i + 1; j < n; j++) {
                ICirculinearElement2D elem2 = curve.get(j);
     ...
     */
    private static boolean isCommonVertex(Point2D inter, ICirculinearCurve2D elem1, ICirculinearCurve2D elem2) {

        double eps = IShape2D.ACCURACY;

        // Test end of elem1 and start of elem2
        if (!Double.isInfinite(elem1.t1()) && !Double.isInfinite(elem2.t0()))
            if (inter.almostEquals(elem1.lastPoint(), eps) && inter.almostEquals(elem2.firstPoint(), eps))
                return true;

        // Test end of elem2 and start of elem1
        if (!Double.isInfinite(elem1.t0()) && !Double.isInfinite(elem2.t1()))
            if (inter.almostEquals(elem1.firstPoint(), eps) && inter.almostEquals(elem2.lastPoint(), eps))
                return true;
     ...
     */
    public static Collection<Point2D> findIntersections(ICirculinearCurve2D curve1, ICirculinearCurve2D curve2) {

        // create array of circulinear elements
        ArrayList<ICirculinearElement2D> elements1 = new ArrayList<ICirculinearElement2D>();
        ArrayList<ICirculinearElement2D> elements2 = new ArrayList<ICirculinearElement2D>();

        // extract all circulinear elements of the curve
        for (ICirculinearContinuousCurve2D cont : curve1.continuousCurves())
            elements1.addAll(cont.smoothPieces());
        for (ICirculinearContinuousCurve2D cont : curve2.continuousCurves())
        ...
        for (int i = 0; i < n1; i++) {
            ICirculinearElement2D elem1 = elements1.get(i);
            for (int j = 0; j < n2; j++) {
                ICirculinearElement2D elem2 = elements2.get(j);
                // iterate on intersection between consecutive elements
                for (Point2D inter : findIntersections(elem1, elem2)) {
                    // add the intersection if we keep it
                    result.add(inter);
     ...
     */
    public static double[][] locateIntersections(ICirculinearCurve2D curve1, ICirculinearCurve2D curve2) {
     ...
     */
    public static Collection<Point2D> findIntersections(ICirculinearElement2D elem1, ICirculinearElement2D elem2) {

        // find which shapes are linear
        boolean b1 = elem1 instanceof ILinearShape2D;
        boolean b2 = elem2 instanceof ILinearShape2D;

        // if both elements are linear, check parallism to avoid computing
        // intersection of parallel lines
        if (b1 && b2) {
            ILinearShape2D line1 = (ILinearShape2D) elem1;
            ILinearShape2D line2 = (ILinearShape2D) elem2;
        ...
        if (elem1 instanceof ILinearShape2D) {
            return elem2.intersections((ILinearShape2D) elem1);
        }
        if (elem2 instanceof ILinearShape2D) {
            return elem1.intersections((ILinearShape2D) elem2);
        }

        // From now, both elem1 and elem2 are instances of CircleShape2D
        // It is therefore possible to extract support circles
        Circle2D circ1 = ((ICircularShape2D) elem1).supportingCircle();
        Circle2D circ2 = ((ICircularShape2D) elem2).supportingCircle();
     ...
     */
    public static Collection<ICirculinearContinuousCurve2D> splitContinuousCurve(ICirculinearContinuousCurve2D curve) {
        ...
        PolyCirculinearCurve2D<ICirculinearElement2D> polyCurve = createPolyCurve(curve.smoothPieces(), curve.isClosed());
        ...
        if (couples.length == 0) {
            ...
            result.add(createPolyCurve(polyCurve.smoothPieces(), curve.isClosed()));
            return result;
        }
    ...
    /**
     * This is a helper method, used to avoid excessive use of generics within other methods of the class.
     */
    private static PolyCirculinearCurve2D<ICirculinearElement2D> createPolyCurve(Collection<? extends ICirculinearElement2D> elements, boolean closed) {
        return new PolyCirculinearCurve2D<ICirculinearElement2D>(elements, closed);
    }

    public static Collection<ICirculinearContour2D> splitIntersectingContours(ICirculinearContour2D curve1, ICirculinearContour2D curve2) {
        ...
        while (!twins1.isEmpty()) {
            ...
            // create continuous curve formed only by circulinear elements
            // and add it to the set of curves
            contours.add(BoundaryPolyCirculinearCurve2D.create(elements.toArray(new ICirculinearElement2D[0]), true));
        }
     ...
     */
    public static Collection<ICirculinearContour2D> splitIntersectingContours(Collection<? extends ICirculinearContour2D> curves) {
        ...
        ICirculinearContour2D[] curveArray = curves.toArray(new ICirculinearContour2D[0]);
        ...
        ArrayList<TreeMap<Double, Integer>> twinIndices = new ArrayList<TreeMap<Double, Integer>>(nCurves);
        ...
        // on each curve
        ArrayList<TreeSet<Double>> positions = new ArrayList<TreeSet<Double>>(nCurves);
        ...
        for (int i = 0; i < nCurves - 1; i++) {
            ICirculinearContour2D curve1 = curveArray[i];

            for (int j = i + 1; j < nCurves; j++) {
                ICirculinearContour2D curve2 = curveArray[j];
                double[][] couples = locateIntersections(curve1, curve2);
        ...
        for (int i = 0; i < nCurves; i++) {
            ...
            ICirculinearContour2D curve0 = curveArray[i];
            ...
            do {
                // the current contour
                ICirculinearContour2D curve = curveArray[ind];
                ...
                if ((pos2 < pos1) && !curve.isBounded()) {
                    ...
                    addElements(elements, curve.subCurve(pos1, curve.t1()));
                } else {
                    // simple case:
    ...
    /**
     * Add all circulinear elements of the given curve to the collection of circulinear elements.
     */
    private static void addElements(Collection<ICirculinearElement2D> elements, ICirculinearContinuousCurve2D curve) {
    ...
    public static double getDistanceCurvePoints(ICirculinearCurve2D curve, Collection<? extends Point2D> points) {
