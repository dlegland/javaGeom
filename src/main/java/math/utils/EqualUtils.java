...
/**
 * Collected methods which allow easy implementation of <code>equals</code>. Rewritten from http://www.javapractices.com/topic/TopicAction.do?Id=17.
 *
 ...
 *         return false;
 *     Car that = (Car) aThat;
 *     return EqualsUtil.areEqual(this.name, that.name) && EqualsUtil.areEqual(this.numDoors, that.numDoors) && EqualsUtil.areEqual(this.gasMileage, that.gasMileage) && EqualsUtil.areEqual(this.color, that.color) && Arrays.equals(this.maintenanceChecks, that.maintenanceChecks); // array!
 * }
 * </pre>
 *
...
public final class EqualUtils {
    ...
    /**
     * Implements <= for doubles based on approximate equality. Approximate equality is defined by Shape2D.ACCURACY.
     * 
     * @param aThis
     * @param aThat
    ...
    static public boolean isLE(double aThis, double aThat) {
        ...
        if (Math.abs(aThat - aThis) <= IShape2D.ACCURACY)
            return true;
