...
public class BoundaryPolyCurve2DTest extends TestCase {
    ...
    public void testTransform_Motion() {
        ...
        IBoundary2D boundary = BoundaryPolyCurve2D.create(new LineSegment2D[] { seg1, seg2, seg3 });
        IDomain2D domain = new GenericDomain2D(boundary);

        AffineTransform2D trans = AffineTransform2D.createRotation(new Point2D(0, 0), .1);
        ...
        IDomain2D domain2 = domain.transform(trans);
    ...
    public void testIsInside_Wedge() {
        ...
        BoundaryPolyCurve2D<AbstractLine2D> curve = BoundaryPolyCurve2D.create(new InvertedRay2D(point, v1), new Ray2D(point, v2));
    ...
    /**
     * Test method for {@link math.geom2d.domain.PolyOrientedCurve2D#signedDistance(double, double)}. Test the method getSignedDistance. Use a clinical case based on line segments. The test point is outside the curve, but inside the domain bounded by the 'closest' curve. Actually, 2 curves are closest.
     */
    public void testIsInsidePoint2D() {
        ...
        IBoundary2D reverse = boundary.reverse();
        ...
        // Try on the inverse curve
        IBoundary2D reverse2 = boundary.reverse();
