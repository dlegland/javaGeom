...
public class Parabola2DTest extends TestCase {
    ...
    public void testClipLine2D() {
        ...
        ICurveSet2D<?> clippedCurve;

        clippedCurve = Curves2DUtil.clipSmoothCurve(parabola, line);
        ...
        assertTrue(curve instanceof ParabolaArc2D);
        assertTrue(new ParabolaArc2D(parabola, -2, 2).almostEquals(curve, IShape2D.ACCURACY));
        ...
        curve = clippedCurve.firstCurve();
        assertTrue(curve instanceof ParabolaArc2D);
        assertTrue(new ParabolaArc2D(parabola, 0, 2).almostEquals(curve, IShape2D.ACCURACY));
        ...
        curve = clippedCurve.firstCurve();
        assertTrue(curve instanceof ParabolaArc2D);
        assertTrue(new ParabolaArc2D(parabola, -2, 4).almostEquals(curve, IShape2D.ACCURACY));

        // Oblic parabola with horizontal line
        double theta = Angle2DUtil.formatAngle(-Math.atan(2));
    ...
    public void testClipBox2D() {
        ...
        ICurveSet2D<ParabolaArc2D> clippedCurve = parabola.clip(box);
        ICurve2D curve = clippedCurve.firstCurve();
    ...
    public void testTransformAffineTransform2D() {
        ...
        assertTrue(scaled.almostEquals(new Parabola2D(0, 0, a * s, 0), IShape2D.ACCURACY));

        double theta = Math.PI / 3;
        Parabola2D rotated = base.transform(AffineTransform2D.createRotation(theta));
        assertTrue(rotated.almostEquals(new Parabola2D(0, 0, a, theta), IShape2D.ACCURACY));

        double dx = 2, dy = 3;
        Parabola2D translated = base.transform(AffineTransform2D.createTranslation(dx, dy));
        assertTrue(translated.almostEquals(new Parabola2D(dx, dy, a, 0), IShape2D.ACCURACY));

        Parabola2D reflected = base.transform(AffineTransform2D.createLineReflection(new StraightLine2D(new Point2D(0, 0), new Vector2D(1, 0))));
        assertTrue(reflected.almostEquals(new Parabola2D(0, 0, -a, 0), IShape2D.ACCURACY));
    }
