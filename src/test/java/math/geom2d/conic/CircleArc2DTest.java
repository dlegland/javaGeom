...
public class CircleArc2DTest extends TestCase {
    ...
    /**
     * Create circle arc with various constructors, and check whether they are the same.
     */
    ...
    public void testTransformInversion() {
        ...
        ICirculinearCurve2D res = arc.transform(inv);
        assertNotNull(res);
    ...
    public void testGetLength() {
        CircleArc2D arc = new CircleArc2D(new Point2D(0, 0), 10, 0, PI / 2);
        assertEquals(arc.length(), (10 * PI / 2), IShape2D.ACCURACY);
    }

    public void testGetT0Double() {
        CircleArc2D arc = new CircleArc2D(new Point2D(0, 0), 10, 0, PI / 2);
        assertEquals(arc.t0(), 0, IShape2D.ACCURACY);
        assertEquals(arc.t1(), PI / 2, IShape2D.ACCURACY);
    }
    ...
    public void testGetPointDouble() {
        ...
        assertTrue(p1.distance(arc.point(arc.t0())) < IShape2D.ACCURACY);
        Point2D p2 = new Point2D(0, 10);
        assertTrue(p2.distance(arc.point(arc.t1())) < IShape2D.ACCURACY);
    ...
    /*
     * Test for double getDistance(double, double). Use a circle arc centered on origin, from 0 to pi/2 radians, and check distances with points on (0,0), (r,0), (0,r), (r,r).
     */
    public void testGetDistancedoubledouble() {
        CircleArc2D arc = new CircleArc2D(new Point2D(0, 0), 10, 0, PI / 2);
        assertEquals(arc.distance(0, 0), 10, IShape2D.ACCURACY);
        assertEquals(arc.distance(10, 0), 0, IShape2D.ACCURACY);
        assertEquals(arc.distance(0, 10), 0, IShape2D.ACCURACY);
        assertEquals(arc.distance(10, -10), 10, IShape2D.ACCURACY);
        assertEquals(arc.distance(10, 10), (10 * (Math.sqrt(2) - 1)), IShape2D.ACCURACY);
    }
    ...
    public void testGetTangent() {
        ...
        assertVectorEquals(new Vector2D(-10, 0), tangent, IShape2D.ACCURACY);
    ...
    public void testGetWindingAngle() {
        ...
        assertEquals(arc.windingAngle(p), (PI / 2), IShape2D.ACCURACY);
        p = new Point2D(0, -10);
        assertEquals(arc.windingAngle(p), (PI / 4), IShape2D.ACCURACY);
        p = new Point2D(0, -20);
        assertEquals(arc.windingAngle(p), Math.atan(.5), IShape2D.ACCURACY);
    }
    ...
    public void testClipBox2D() {
        ...
        assertTrue(clip1 instanceof ICurveSet2D<?>);
        assertEquals(((ICurveSet2D<?>) clip1).size(), 0);
        ...
        assertTrue(clip1 instanceof ICurveSet2D<?>);
        assertEquals(((ICurveSet2D<?>) clip1).size(), 1);
        ICurve2D clip1c1 = ((ICurveSet2D<?>) clip1).firstCurve();
        assertTrue(clip1c1 instanceof CircleArc2D);

        // a circle arc clipped with both extremities outside the rectangle
        IShape2D curves = arc2.clip(rect1);
        // assertTrue(curves.getClass().equals(CurveSet2D.class));
        ICurve2D curve1 = ((ICurveSet2D<?>) curves).firstCurve();
        ...
        // assertTrue(curves.getClass().equals(CurveSet2D.class));
        curve1 = ((ICurveSet2D<?>) curves).firstCurve();
        assertTrue(curve1.getClass().equals(CircleArc2D.class));
    ...
    public void testClipBox2DInverse() {
        ...
        IShape2D clipped;
        ICurveSet2D<CircleArc2D> curves;

        // a box totally outside the arc
        clipped = arc1.clip(boxOut);
        assertTrue(clipped instanceof ICurveSet2D<?>);
        assertEquals(((ICurveSet2D<?>) clipped).size(), 0);
        ...
        assertTrue(clipped instanceof ICurveSet2D<?>);
        assertEquals(((ICurveSet2D<?>) clipped).size(), 1);
        ICurve2D clip1c1 = ((ICurveSet2D<?>) clipped).firstCurve();
        assertTrue(clip1c1 instanceof CircleArc2D);
        ...
        // assertTrue(curves.getClass().equals(CurveSet2D.class));
        ICurve2D curve1 = ((ICurveSet2D<?>) clipped).firstCurve();
        assertTrue(curve1.getClass().equals(CircleArc2D.class));
        // Shape2D arc2c = arc2.clip(rect1);
        // System.out.println(arc2c);
        assertTrue(curve1.equals(arc1));
        ...
        assertTrue(iter.next().almostEquals(arc1, IShape2D.ACCURACY));
        assertTrue(iter.next().almostEquals(arc2, IShape2D.ACCURACY));
    }
    ...
    public void testGetReverseCurve() {
        ...
        assertTrue(rev1.almostEquals(exp1, IShape2D.ACCURACY));
        ...
        assertTrue(arc2.reverse().almostEquals(new CircleArc2D(circle2, PI / 2, PI / 2), IShape2D.ACCURACY));

        // direct arc, indirect circle
        CircleArc2D arc3 = new CircleArc2D(circle2, 0, PI / 2);
        assertTrue(arc3.reverse().almostEquals(new CircleArc2D(circle2, PI / 2, -PI / 2), IShape2D.ACCURACY));

        // inverse circle arc, direct circle
        CircleArc2D arc4 = new CircleArc2D(circle1, PI, -PI / 2);
        assertTrue(arc4.reverse().almostEquals(new CircleArc2D(circle1, PI / 2, PI / 2), IShape2D.ACCURACY));
    }

    public void testClass() {
        Point2D origin = new Point2D(0, 0);
        CircleArc2D arc = new CircleArc2D(origin, 10, 0, PI / 2);
        assertTrue(ICurve2D.class.isInstance(arc));
    }
    ...
    public void testTransform_Translation2D() {
        ...
        IEllipseArcShape2D res = arc.transform(tra);

        // basic tests
        assertTrue(res instanceof CircleArc2D);
        CircleArc2D exp = new CircleArc2D(tx, ty, 10, 0, PI / 2);
        assertTrue(res.almostEquals(exp, IShape2D.ACCURACY));
    }
    ...
    public void testTransform_Rotation2D() {
        ...
        IEllipseArcShape2D res = arc.transform(rot);
    ...
    public void testTransform_Scaling2D() {
        ...
        IEllipseArcShape2D res = arc.transform(sca);
    ...
    public void testTransform_LineReflectionOx() {
        ...
        IEllipseArcShape2D res = arc.transform(trans);
    ...
    public void testTransformAffineTransform2DInv() {
        ...
        // translation
        AffineTransform2D tra = AffineTransform2D.createTranslation(tx, ty);
        assertTrue(arc.transform(tra).almostEquals(new CircleArc2D(x0 + tx, y0 + ty, r, PI / 2, -PI / 2), IShape2D.ACCURACY));

        // rotation
        AffineTransform2D rot = AffineTransform2D.createRotation(p0, theta);
        assertTrue(arc.transform(rot).almostEquals(new CircleArc2D(x0, y0, r, theta + PI / 2, -PI / 2), IShape2D.ACCURACY));

        // scaling with unequal factors
        AffineTransform2D sca = AffineTransform2D.createScaling(p0, sx, sy);
        assertTrue(arc.transform(sca).equals(new EllipseArc2D(x0, y0, 3 * r, 2 * r, 0, PI / 2, -PI / 2)));

        // line reflections
        AffineTransform2D refOx = AffineTransform2D.createLineReflection(new StraightLine2D(x0, y0, 1, 0));
        IEllipseArcShape2D transformed = arc.transform(refOx);
        assertTrue(transformed.almostEquals(new CircleArc2D(x0, y0, r, 3 * PI / 2, PI / 2), IShape2D.ACCURACY));
        AffineTransform2D refOy = AffineTransform2D.createLineReflection(new StraightLine2D(x0, y0, 0, 1));
        assertTrue(arc.transform(refOy).almostEquals(new CircleArc2D(x0, y0, r, PI / 2, PI / 2), IShape2D.ACCURACY));
    }
