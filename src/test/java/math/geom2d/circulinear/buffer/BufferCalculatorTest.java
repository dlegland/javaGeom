...
import math.geom2d.circulinear.ICirculinearCurveSet2D;
...
/**
 * Test computation of buffer and parallel on a variety of circulinear curves.
 */
public class BufferCalculatorTest extends TestCase {
    ...
    public void testCreateParallel_LineSegment() {
        LineSegment2D curve = new LineSegment2D(new Point2D(100, 100), new Point2D(200, 100));

        BufferCalculator bc = BufferCalculator.getDefaultInstance();
        ICirculinearCurve2D parallel = bc.createParallel(curve, 20);
        ...
        ICirculinearCurve2D parallel2 = bc.createParallel(curve, -20);
    ...
    public void testCreateParallel_TwoLineSegments() {
        LineSegment2D line1 = new LineSegment2D(new Point2D(100, 100), new Point2D(200, 100));
        LineSegment2D line2 = new LineSegment2D(new Point2D(100, 200), new Point2D(200, 200));

        ICirculinearCurve2D curve = CirculinearCurveArray2D.create(new LineSegment2D[] { line1, line2 });
    ...
    public void testCreateParallelContour_StraightLine() {
        StraightLine2D curve = new StraightLine2D(new Point2D(100, 100), new Point2D(200, 100));

        BufferCalculator bc = BufferCalculator.getDefaultInstance();
        ICirculinearContour2D parallel = bc.createParallelContour(curve, 20);
        ...
        ICirculinearContour2D parallel2 = bc.createParallelContour(curve, -20);
    ...
    public void testCreateParallelContour_LinearRing() {
        LinearRing2D curve = new LinearRing2D(new Point2D[] { new Point2D(100, 100), new Point2D(200, 100), new Point2D(200, 200) });
    ...
    public void testCreateParallelContour_ColinearPolyline() {
        ...
        Polyline2D curve = new Polyline2D(new Point2D[] { new Point2D(100, 100), new Point2D(150, 100), new Point2D(200, 100) });

        BufferCalculator bc = BufferCalculator.getDefaultInstance();
        ICirculinearContinuousCurve2D parallel = bc.createContinuousParallel(curve, 20);

        assertNotNull(parallel);
        assertFalse(parallel.isEmpty());
        assertEquals(2, parallel.smoothPieces().size());

        ICirculinearContinuousCurve2D parallel2 = bc.createContinuousParallel(curve, -20);
    ...
    public void testCreateParallelBoundary_TwoCircles() {
        ...
        ICirculinearBoundary2D curve = CirculinearContourArray2D.create(new Circle2D[] { circle1, circle2 });

        BufferCalculator bc = BufferCalculator.getDefaultInstance();
        ICirculinearBoundary2D parallel = bc.createParallelBoundary(curve, 20);
        ...
        ICirculinearBoundary2D parallel2 = bc.createParallelBoundary(curve, -20);
    ...
    public void testCreateContinousParallel_CircleArcInverse() {
        ...
        // compute parallel
        ICirculinearContinuousCurve2D parallel = bc.createContinuousParallel(arc, 10);
        ...
        ICirculinearContinuousCurve2D parallel2 = bc.createContinuousParallel(arc, -10);
    ...
    public void testCreateContinousParallel_InfiniteCurve() {
        ...
        // compute parallel
        ICirculinearContinuousCurve2D parallel = bc.createContinuousParallel(line, 10);
        assertNotNull(parallel);
        assertEquals(1, parallel.continuousCurves().size());
        assertFalse(parallel.isEmpty());
        assertEquals(1, parallel.smoothPieces().size());
        ...
        // same in other direction
        ICirculinearContinuousCurve2D parallel2 = bc.createContinuousParallel(line, -10);
        assertNotNull(parallel2);
        assertEquals(1, parallel2.continuousCurves().size());
        assertFalse(parallel2.isEmpty());
        assertEquals(1, parallel2.smoothPieces().size());
    ...
    public void testCreateContinousParallel_BiRay() {
        ...
        BufferCalculator bc = BufferCalculator.getDefaultInstance();
        ICirculinearContinuousCurve2D parallel = bc.createContinuousParallel(curve, 10);
        assertNotNull(parallel);
        assertFalse(parallel.isEmpty());
        assertEquals(3, parallel.smoothPieces().size());

        // the same in opposite direction
        ICirculinearContinuousCurve2D parallel2 = bc.createContinuousParallel(curve, 10);
        assertNotNull(parallel2);
        assertFalse(parallel2.isEmpty());
        assertEquals(3, parallel2.smoothPieces().size());

        Collection<ICirculinearContinuousCurve2D> splittedCurves = CirculinearCurves2D.splitContinuousCurve(parallel2);
    ...
    public void testCreateContinuousParallel_Astroid() {
        ...
        // computes the parallel
        BufferCalculator bc = BufferCalculator.getDefaultInstance();
        ICirculinearContinuousCurve2D parallel = bc.createContinuousParallel(astroid, 10);

        assertNotNull(parallel);
        assertFalse(parallel.isEmpty());
        assertEquals(8, parallel.smoothPieces().size());

        for (ICirculinearCurve2D curve : parallel.smoothPieces()) {
    ...
    public void testComputeBuffer_LineSegment() {
        // polyline with two edges that are colinear
        LineSegment2D line = new LineSegment2D(new Point2D(100, 100), new Point2D(200, 100));

        BufferCalculator bc = BufferCalculator.getDefaultInstance();
        ICirculinearDomain2D buffer = bc.computeBuffer(line, 20);
    ...
    public void testComputeBuffer_ColinearPolyline() {
        // polyline with two edges that are colinear
        Polyline2D curve = new Polyline2D(new Point2D(100, 100), new Point2D(150, 100), new Point2D(200, 100));
    ...
    public void testComputeBuffer_StraightLine() {
        ...
        ICirculinearCurve2D curve = new StraightLine2D(new Point2D(10, 20), new Point2D(30, 50));
        double dist = 30;

        IDomain2D buffer = bc.computeBuffer(curve, dist);
    ...
    public void testComputeBuffer_BiRay() {
        ...
        // computes the buffer
        BufferCalculator bc = BufferCalculator.getDefaultInstance();
        ICirculinearDomain2D buffer = bc.computeBuffer(curve, 10);
        assertNotNull(buffer);
        assertFalse(buffer.isEmpty());
    ...
    public void testComputeBuffer_TwoLines() {
        ...
        // put lines in a set
        ICirculinearCurveSet2D<StraightLine2D> set = CirculinearCurveArray2D.create(line1, line2);

        // compute set buffer and buffer boundary
        BufferCalculator bc = BufferCalculator.getDefaultInstance();
        ICirculinearDomain2D buffer = bc.computeBuffer(set, 10);
    ...
    public void testComputeBuffer_TwoCrossingLines() {
        ...
        StraightLine2D line1 = new StraightLine2D(new Point2D(10, 20), new Point2D(10, 0));
        StraightLine2D line2 = new StraightLine2D(new Point2D(20, 10), new Point2D(0, 10));
        ICirculinearCurve2D curve = CirculinearCurveArray2D.create(line1, line2);
        double dist = 3;
    ...
    public void testComputeBuffer_TwoCrossingRays() {
        ...
        ICirculinearCurve2D curve = CirculinearCurveArray2D.create(ray1, ray2);
        double dist = 3;
    ...
    public void testComputeBuffer_PolyRay() {
        ...
        // create the curve
        ICirculinearCurve2D curve = PolyCirculinearCurve2D.create(ray1, edge, ray2);
