...
public class CirculinearCurves2DTest extends TestCase {
    ...
    public void testConvert_PolyCurve() {
        ...
        PolyCurve2D<LineSegment2D> curve = PolyCurve2D.create(new LineSegment2D[] { seg1, seg2 });

        converted = CirculinearCurves2D.convert(curve);
        assertTrue(converted instanceof PolyCirculinearCurve2D);

        Collection<? extends ISmoothCurve2D> smoothCurves = converted.continuousCurves().iterator().next().smoothPieces();
    ...
    public void testConvert_PolyCurveWithSpline() {
        ...
        LineSegment2D seg1 = new LineSegment2D(p1, p2);
        LineSegment2D seg2 = new LineSegment2D(p2, p3);
        CubicBezierCurve2D bezier = new CubicBezierCurve2D(p3, new Point2D(0, 10), new Point2D(10, 50), new Point2D(20, 50));
        PolyCurve2D<ISmoothCurve2D> curve = PolyCurve2D.create(new ISmoothCurve2D[] { seg1, seg2, bezier });
    ...
    public void testConvert_CurveSet() {
        ...
        CurveArray2D<LineSegment2D> curve = CurveArray2D.create(new LineSegment2D[] { seg1, seg2 });

        res = CirculinearCurves2D.convert(curve);
        assertTrue(res instanceof ICirculinearCurveSet2D);

        Collection<? extends IContinuousCurve2D> smoothCurves = res.continuousCurves();
    ...
    public void testConvert_CurveSetWithSpline() {
        ...
        CurveArray2D<ISmoothCurve2D> curve = CurveArray2D.create(new ISmoothCurve2D[] { seg1, seg2, bezier });
    ...
    public void testSplitContinuousCurve() {
        // elements
        LineSegment2D edge1 = new LineSegment2D(new Point2D(50, 100), new Point2D(150, 100));
        CircleArc2D arc1 = new CircleArc2D(new Point2D(100, 100), 50, 0, Math.PI / 2);
        LineSegment2D edge2 = new LineSegment2D(new Point2D(100, 150), new Point2D(100, 50));

        // create the curve
        ICirculinearContinuousCurve2D curve = PolyCirculinearCurve2D.create(new ICirculinearElement2D[] { edge1, arc1, edge2 });

        // split the curve
        Collection<ICirculinearContinuousCurve2D> set = CirculinearCurves2D.splitContinuousCurve(curve);
    ...
    public void testSplitIntersectingContours_Circles() {
        ...
        Collection<? extends ICirculinearContour2D> contours = CirculinearCurves2D.splitIntersectingContours(circle1, circle2);
        assertEquals(contours.size(), 2);
    }

    public void testSplitIntersectingContours_Rectangles() {
        LinearRing2D poly1 = new LinearRing2D(new Point2D[] { new Point2D(10, 20), new Point2D(40, 20), new Point2D(40, 30), new Point2D(10, 30) });
        LinearRing2D poly2 = new LinearRing2D(new Point2D[] { new Point2D(10, 20), new Point2D(40, 20), new Point2D(40, 30), new Point2D(10, 30) });

        Collection<ICirculinearContour2D> contours = CirculinearCurves2D.splitIntersectingContours(poly1, poly2);
        assertEquals(contours.size(), 2);
    }
    ...
    public void testSplitIntersectingContours_3Circles() {
        ...
        Collection<ICirculinearContour2D> result = CirculinearCurves2D.splitIntersectingContours(rings);
    ...
    public void testSplitIntersectingContours_4Lines() {
        ...
        // put lines in a set
        ICirculinearCurveSet2D<StraightLine2D> set = CirculinearCurveArray2D.create(new StraightLine2D[] { line1, line2, line3, line4 });

        Collection<StraightLine2D> curves = set.curves();
        Collection<ICirculinearContour2D> contours = CirculinearCurves2D.splitIntersectingContours(curves);
        assertEquals(5, contours.size());
    }

    public void testFindSelfIntersections() {
        // create polyline
        LinearRing2D line = new LinearRing2D(new Point2D[] { new Point2D(100, 0), new Point2D(0, 0), new Point2D(0, 100), new Point2D(200, 100), new Point2D(200, 200), new Point2D(100, 200) });

        // detection of intersections
        Collection<Point2D> points = CirculinearCurves2D.findSelfIntersections(line);
    ...
    public void testSplitContinuousCurveParallelBiRay() {
        ...
        ICirculinearContinuousCurve2D parallel = curve.parallel(10);
        Collection<ICirculinearContinuousCurve2D> splittedCurves = CirculinearCurves2D.splitContinuousCurve(parallel);

        assertFalse(splittedCurves.isEmpty());
        assertEquals(1, splittedCurves.size());

        // computes the negative parallel
        ICirculinearContinuousCurve2D parallel2 = curve.parallel(-10);
        Collection<Point2D> points = parallel2.singularPoints();
        assertEquals(2, points.size());

        PointArray2D pointSet = PointArray2D.create(points);
        assertTrue(pointSet.distance(new Point2D(18, 4)) < IShape2D.ACCURACY);
        assertTrue(pointSet.distance(new Point2D(4, 18)) < IShape2D.ACCURACY);
    }
